...
# FRONT: todo lo visual está marcado — usar C["clave"] para colores
...
class JuegoUno(ctk.CTkToplevel):
    ...
    def _build_ui(self):
        ...
        # FRONT: título de la sección
        ctk.CTkLabel(
            header,
            text=f"Juego 1 — {p.CAMINO[self.seccion_id]['nombre']}",
            font=ctk.CTkFont(size=16, weight="bold"),
            text_color=C["texto_principal"]
        ).pack(side="left")

        # FRONT: contador de preguntas
        self.lbl_progreso = ctk.CTkLabel(
            header,
            text=f"0 / {PREGUNTAS_PARA_PASAR}",
            font=ctk.CTkFont(size=13),
            text_color=C["texto_secundario"]
        )
        self.lbl_progreso.pack(side="right")

        # FRONT: barra de progreso
        ...
        self.barra.set(0)
        self.barra.pack(fill="x", padx=24, pady=(10, 0))

        # FRONT: feedback correcto/incorrecto
        self.lbl_feedback = ctk.CTkLabel(
            self, text="¿Qué letra es esta seña?",
            font=ctk.CTkFont(size=15),
            text_color=C["texto_principal"]
        )
        self.lbl_feedback.pack(pady=(20, 8))

        # FRONT: imagen de la seña — ajustar tamaño con size en _cargar_imagen
        self.lbl_imagen = ctk.CTkLabel(
            self, text="",
            width=260, height=260,
        )
        self.lbl_imagen.pack(pady=(0, 20))

        # FRONT: 4 botones en grid 2x2
        frame_btns = ctk.CTkFrame(self, fg_color=C["fondo"])
    ...
    def _cargar_imagen(self, letra):
        ...
        img = Image.open(ruta).resize((260, 260))
        img_ctk = ctk.CTkImage(light_image=img, dark_image=img, size=(260, 260))
    ...
    def _verificar(self, idx):
        ...
        if letra_sel == self.letra_actual:
            self.aciertos += 1
            self.botones[idx].configure(
                fg_color=C["completado"], border_color=C["completado"]
            )
            self.lbl_feedback.configure(
                text="¡Correcto!", text_color=C["completado"]
            )
        else:
            self.botones[idx].configure(
                fg_color=C["error"], border_color=C["error"]
            )
            for btn in self.botones:
            ...
            self.lbl_feedback.configure(
                text=f"Era la '{self.letra_actual}'", text_color=C["error"]
            )

        self.lbl_progreso.configure(
            text=f"{self.preguntas_hechas} / {PREGUNTAS_PARA_PASAR}"
        )
